...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'reports' | 'finances' | 'security' | 'inventory' | 'menu' | 'analytics' | 'reports_export'>('reports');
  ...
  const handleSaveExpenseForm = async (e: React.FormEvent) => {
    ...
    const formData = new FormData(e.target as HTMLFormElement);
    const selectedDate = expenseDate || new Date().toISOString().slice(0, 10);
    ...
    <div className="flex h-screen bg-[#FDFCF9] font-sans overflow-hidden text-[#4B3621]">
      <aside className="w-[340px] bg-white border-r border-gray-100 flex flex-col shrink-0 z-20 shadow-lg">
        ...
        <nav className="flex-1 px-6 space-y-3 py-6 overflow-y-auto scrollbar-thin">
          ...
          <NavLink id="analytics" label="Deep Analytics" icon={Activity} />
          <NavLink id="reports_export" label="Reports & Export" icon={Download} />
        </nav>
                      ...
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td className="px-12 py-10 text-right">
                          <div className="flex justify-end gap-2 flex-wrap sm:flex-nowrap">
                            <button onClick={() => onUpdateStock(item.id, -1)} className="w-11 h-11 flex items-center justify-center bg-white border-2 border-gray-50 rounded-[20px] text-red-400 hover:text-red-600 hover:shadow-xl transition-all active:scale-90 shadow-sm"><Minus size={18}/></button>
                            <button onClick={() => onUpdateStock(item.id, 1)} className="w-11 h-11 flex items-center justify-center bg-white border-2 border-gray-50 rounded-[20px] text-teal-500 hover:text-teal-700 hover:shadow-xl transition-all active:scale-90 shadow-sm"><Plus size={18}/></button>
                            <button onClick={() => { setEditingInvItem(item); setIsInvModalOpen(true); }} className="w-11 h-11 flex items-center justify-center bg-[#4B3621] text-white rounded-[20px] shadow-2xl hover:scale-105 active:scale-95 transition-all"><Edit3 size={18}/></button>
                            <button onClick={() => onDeleteInventoryItem(item.id)} className="w-11 h-11 flex items-center justify-center bg-red-50 text-red-500 rounded-[20px] hover:bg-red-500 hover:text-white transition-all"><Trash2 size={18}/></button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="bg-white rounded-[56px] shadow-sm border border-gray-50 overflow-hidden">
                <div className="p-12 border-b border-gray-50 bg-gray-50/20 space-y-8">
                  <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-6">
                    <div>
                      <p className="text-[10px] font-black text-red-400 uppercase tracking-[4px] mb-3">Finance Control</p>
                      <h3 className="font-serif text-4xl font-black text-[#4B3621] tracking-tighter">Expense Ledger</h3>
                      <p className="text-sm text-gray-400 font-bold mt-3">Track cash outflow by the day it was lodged, not just the day it was viewed.</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full xl:w-auto">
                      <div className="bg-white px-6 py-4 rounded-[24px] border border-gray-100 shadow-sm">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Visible Period</p>
                        <span className="text-sm font-black text-[#4B3621]">{dateRange}</span>
                      </div>
                      <div className="bg-white px-6 py-4 rounded-[24px] border border-gray-100 shadow-sm">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Ledger Count</p>
                        <span className="text-sm font-black text-[#4B3621]">{filteredExpenses.length} entries</span>
                      </div>
                      <div className="bg-white px-6 py-4 rounded-[24px] border border-gray-100 shadow-sm">
                        <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1">Period Outflow</p>
                        <span className="text-2xl font-black text-red-500">{CURRENCY} {reportStats.totalExp.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-5">
                    <div className="flex flex-wrap gap-2 bg-white p-2 rounded-[24px] border border-gray-100 shadow-sm w-fit max-w-full overflow-x-auto">
                      {['Day', 'Yesterday', 'Week', 'Month', 'All', 'Custom'].map(r => (
                        <button
                          key={r}
                          onClick={() => setDateRange(r as any)}
                          className={`px-6 py-3 rounded-[18px] text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${dateRange === r ? 'bg-[#4B3621] text-white shadow-xl' : 'text-gray-400 hover:text-[#4B3621]'}`}
                        >
                          {r}
                        </button>
                      ))}
                    ...
                    {dateRange === 'Custom' && (
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-white p-3 rounded-[24px] border border-gray-100 shadow-sm">
                        <input
                          type="date"
                          value={customStart}
                          onChange={e => setCustomStart(e.target.value)}
                          className="bg-gray-50 border-none rounded-xl text-xs font-black p-3 outline-none"
                        />
                        <span className="text-gray-300 font-black self-center">to</span>
                        <input
                          type="date"
                          value={customEnd}
            ...
            <div className="bg-white w-full max-w-xl rounded-[64px] p-16 shadow-2xl relative animate-in zoom-in-95 duration-300">
              ...
              <form onSubmit={handleSaveExpenseForm} className="space-y-10">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    <label className="text-[10px] font-black text-gray-300 uppercase tracking-[4px] mb-4 block">Ledger Description</label>
                    <input name="description" className="w-full p-6 bg-gray-50 border-2 border-gray-100 rounded-[28px] font-black text-lg outline-none" required />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-gray-300 uppercase tracking-[4px] mb-4 block">Date Lodged</label>
                    <input
                      type="date"
                      value={expenseDate}
                      onChange={e => setExpenseDate(e.target.value)}
                      className="w-full p-6 bg-gray-50 border-2 border-gray-100 rounded-[28px] font-black text-lg outline-none"
                      required
                    />
                  </div>
                </div>
