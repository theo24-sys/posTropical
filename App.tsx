...
const App: React.FC = () => {
  ...
  const applyInventoryDeductionLocally = (saleItems: { id: string; quantity: number }[]) => {
    ...
    saleItems.forEach(item => {
      ...
      recipes.forEach(recipe => {
        const amount = recipe.amount * item.quantity;
        if (amount <= 0) return;
        deductionMap.set(recipe.invId, (deductionMap.get(recipe.invId) || 0) + amount);
      });
    });
    ...
    setInventory(prev => prev.map(item => {
      const deduction = deductionMap.get(item.id);
      if (deduction === undefined) return item;
      return {
        ...item,
  ...
  useEffect(() => {
    ...
    const unsubscribe = DB.subscribeToInventory(
      (updatedItem) => {
        setInventory(prev => {
        ...
        LocalDB.saveInventoryItem(updatedItem).catch(() => {});
      },
      (deletedId) => {
        setInventory(prev => prev.filter(item => item.id !== deletedId));
        LocalDB.deleteInventoryItem(deletedId).catch(() => {});
      }
    );
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex gap-2 shrink-0">
          <button
            ...
            className={`flex-1 p-4 rounded-2xl flex items-center justify-center transition-all ${
              location.pathname === '/' ? 'bg-white shadow-lg text-[#14b8a6]' : 'text-gray-300 hover:text-gray-600'
            }`}
            title="POS"
          >
            <LayoutList size={28} />
          ...
          <button
            ...
            className={`flex-1 p-4 rounded-2xl flex items-center justify-center transition-all ${
              location.pathname === '/transactions' ? 'bg-white shadow-lg text-[#14b8a6]' : 'text-gray-300 hover:text-gray-600'
            }`}
          ...
          {isAdmin(posUser) && (
            <button
              ...
              className={`flex-1 p-4 rounded-2xl flex items-center justify-center transition-all ${
                location.pathname === '/admin' ? 'bg-white shadow-lg text-[#14b8a6]' : 'text-gray-300 hover:text-gray-600'
              }`}
              title="Management"
            >
              <BarChart3 size={28} />
